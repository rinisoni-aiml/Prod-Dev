...
import { mockAlerts, mockContracts } from '@/lib/mockData';
...
function AlertsTab() {
  ...
  const filtered = filter === 'all' ? alerts.filter(a => !a.is_resolved)
...
function ContractsTab() {
  ...
  const [newContract, setNewContract] = useState({ contract_name: '', vendor: '', start_date: '', end_date: '', value: '', notes: '' });
  ...
  const stats = [
    ...
    { label: 'Total Value', value: `₹${(contracts.reduce((s, c) => s + c.value, 0) / 100000).toFixed(1)}L`, icon: DollarSign },
  ];
            ...
            <tbody>
              {contracts.map((contract) => (
                <tr key={contract.id} className={`border-b border-border transition-colors ${
                  ...
                  <td className="px-4 py-3 text-foreground">₹{(contract.value / 100000).toFixed(1)}L</td>
                  ...
                  <td className="px-4 py-3">
                    <button onClick={() => setContracts(contracts.filter(c => c.id !== contract.id))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/60 backdrop-blur-sm">
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
            ...
            <button onClick={addContract}
              className="w-full gradient-brand text-primary-foreground py-2.5 rounded-lg font-semibold text-sm mt-4 hover-lift">
              Save Contract
            </button>
          </motion.div>
        </div>
