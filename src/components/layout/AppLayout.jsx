...
import { Bell, ChevronDown, LogOut, Settings, User, LayoutDashboard, TrendingUp, Package, FileText, Upload, PanelLeftClose, PanelLeft, Clock, AlertTriangle, Truck, ShieldCheck, BarChart2, Building2, BookOpen, UserCheck, Users, Briefcase } from 'lucide-react';
import { useState } from 'react';
import Logo from '@/components/Logo';
import ThemeToggle from '@/components/ThemeToggle';
import { useAuthStore } from '@/stores/authStore';
import FMCGChatDrawer from '@/components/chat/AIChatDrawer';
import FMCGChatFAB from '@/components/chat/AIChatFAB';
import EduChatDrawer from '@/industries/education/components/chat/AIChatDrawer';
import EduChatFAB from '@/industries/education/components/chat/AIChatFAB';
...
const educationNavItems = [
  { label: 'Dashboard',       path: '/dashboard/education',            icon: LayoutDashboard },
  { label: 'Academic Intel',  path: '/dashboard/education/academic',   icon: BookOpen },
  { label: 'Admissions',      path: '/dashboard/education/admissions', icon: UserCheck },
  { label: 'Faculty & Staff', path: '/dashboard/education/faculty',    icon: Users },
  { label: 'Placements',      path: '/dashboard/education/placements', icon: Briefcase },
  { label: 'Data Upload',     path: '/dashboard/education/upload',     icon: Upload },
];
        ...
        {!sidebarCollapsed && (
          <div className="p-3 border-t border-border">
            <div className="flex items-center gap-3 px-2 py-2">
              <div className="h-8 w-8 rounded-full gradient-brand flex items-center justify-center text-primary-foreground text-xs font-bold flex-shrink-0">
                {profile?.full_name?.[0] || 'U'}
              </div>
              <div className="min-w-0">
                <p className="text-sm font-medium text-foreground truncate">{profile?.full_name || 'User'}</p>
                <p className="text-xs text-foreground-secondary truncate">{profile?.company_name || 'Company'}</p>
              </div>
            </div>
          </div>
        )}
        ...
        <header className="sticky top-0 z-30 h-14 glass-card border-b flex items-center justify-between px-4">
          ...
          <div className="flex items-center gap-3">
            <div className="hidden sm:flex items-center gap-1.5 text-xs">
              ...
              <span className="text-foreground-secondary">Last updated {timeAgo < 1 ? 'just now' : `${timeAgo} min ago`}</span>
            </div>
            <ThemeToggle />
            <button className="relative h-9 w-9 rounded-lg border border-border bg-background-surface flex items-center justify-center hover-lift">
            ...
            <div className="relative">
              <button onClick={() => setUserMenuOpen(!userMenuOpen)} className="flex items-center gap-2 h-9 px-3 rounded-lg border border-border bg-background-surface hover-lift">
                <div className="h-6 w-6 rounded-full gradient-brand flex items-center justify-center text-primary-foreground text-xs font-bold">
                  {profile?.full_name?.[0] || 'U'}
                </div>
              ...
              {userMenuOpen && (
                <>
                  ...
                  <div className="absolute right-0 top-12 w-48 glass-card rounded-xl border shadow-lg p-2 z-50">
                    <div className="px-3 py-2 border-b border-border mb-1">
                      <p className="text-sm font-medium text-foreground">{profile?.full_name || 'User'}</p>
                      <p className="text-xs text-foreground-secondary">{profile?.company_name || 'Company'}</p>
                    </div>
                    <button onClick={() => { navigate('/profile'); setUserMenuOpen(false); }} className="w-full flex items-center gap-2 px-3 py-2 text-sm text-foreground-secondary hover:text-foreground hover:bg-muted rounded-lg">
                      <User className="h-4 w-4" /> Profile
                    </button>
                    <button onClick={() => { navigate('/settings'); setUserMenuOpen(false); }} className="w-full flex items-center gap-2 px-3 py-2 text-sm text-foreground-secondary hover:text-foreground hover:bg-muted rounded-lg">
                      <Settings className="h-4 w-4" /> Settings
                    </button>
                    <button onClick={handleLogout} className="w-full flex items-center gap-2 px-3 py-2 text-sm text-destructive hover:bg-destructive/10 rounded-lg">
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-40 glass-card border-t">
        <div className="flex items-center justify-around h-14">
          {navItems.slice(0, 4).map(({ label, path, icon: Icon }) => (
            <NavLink key={path} to={path} end className={({ isActive }) => `flex flex-col items-center gap-0.5 px-2 py-1 text-[10px] font-medium transition-colors ${isActive ? 'text-primary' : 'text-foreground-secondary'}`}>
